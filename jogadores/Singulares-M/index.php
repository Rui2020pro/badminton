...
<body>
	...
	<div class="support-buttons">
		...
		<a href="../Singulares-F" class="btn btn-outline-dark"><strong>Singulares-F</strong></a>
	...
	<div class="container p-3">	
			<div class="row" id="links-jogadores">
				...
				$sql = 'SELECT j.nomeJogador, j.imagemJogador,r.rankJogador, j.idJogador 
					FROM jogadores  j,rankings  r
					where  j.idJogador = r.idJogador AND j.modoJogador = "Singulares-M"
				...
				if($stmt && $stmt->rowCount() > 0){
					...
					?>  
						<div class="col-6 col-md-3">
							...
							<a href="../../jogador/?id=<?php echo $obj->idJogador ?>" title=""><img src="../../../gifs/<?php echo $obj->imagemJogador ?>" alt="" class="img-fluid"></a>
							<p><strong>
								<?php 
...
$('#form-enviar-mensagem').submit(function (event){
	...
	//enviar dados
	if(valido == true)
	{ /*chamada mais simples de JQuery*/
		$.ajax({
			type: 'POST',
			url: 'ajax/insereMensagem.php',
			data: { nome: nome/*valor local*/, email: email, mensagem: msg},
			/*beforeSend: function(){
			...
			success: function(data){
				...
				if(data == 0)
				{
					$('#card-form-mensagem').html('<h3>Mensagem recebida.<br>Obrigado.<h3>');
				}
				else
				{		
					$('#card-form-mensagem').html('<h3>De momento indisponível.<br>Tente mais tarde.<h3>');
				}
			},
			error: function(data){
